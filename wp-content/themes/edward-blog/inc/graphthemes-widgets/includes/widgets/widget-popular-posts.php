...
function edward_blog_register_popular_post_widget() {
    register_widget( 'Graphthemes_Widget_Popular_Post' );
}
add_action( 'widgets_init', 'edward_blog_register_popular_post_widget' );
...
class Graphthemes_Widget_Popular_Post extends WP_Widget {
    ...
    function __construct(){
        if( ! is_customize_preview() ) add_action('wp', array( $this, 'edward_blog_set_views' ) );
        
        parent::__construct(
            'edward_blog_widget_popular_post', // Base ID
            esc_html__( 'Graphthemes: Popular Post', 'edward-blog' ), // Name
            array( 'description' => esc_html__( 'A Popular Post Widget', 'edward-blog' ), ) // Args
        );
    }
     ...
     */
    function edward_blog_set_views( $post_id ) {
        if ( in_the_loop() ) {
            $post_id = get_the_ID();
          } 
        else {
        ...
        if( is_singular( 'post' ) )
        {
            $count_key = '_edward_blog_view_count';
     ...
     */
    function edward_blog_get_views( $post_id ){
        $count_key = '_edward_blog_view_count';
        $count = get_post_meta( $post_id, $count_key, true );
        if( $count == '' ){        
            return __( "0 View", 'edward-blog' );
        }elseif($count<=1){
            return $count. __(' View', 'edward-blog' );
        }else{
            return $count. __(' Views', 'edward-blog' );    
        }    
    }
    ...
    public function widget( $args, $instance ) {
        ...
        $title          = ! empty( $instance['title'] ) ?  $instance['title'] : __( 'Popular Posts', 'edward-blog' );
        ...
        if( $based_on == 'views' ){
            ...
            $arg['meta_key'] = '_edward_blog_view_count';
        ...
        $edward_blog_widget_popular_post_size = apply_filters('edward_blog_widget_popular_post_size', 'thumbnail');
        ...
        if( $qry->have_posts() ){
                    ...
                    <li>
                            ...
                            <a <?php echo $target;?> href="<?php the_permalink();?>" class="post-thumbnail">
                                <?php 
                                    if( has_post_thumbnail() && $show_thumbnail ){ 
                                        the_post_thumbnail( $edward_blog_widget_popular_post_size, array( 'itemprop' => 'image' ) );
                                    }
                                ?>
                            </a>
                        <?php }
                        ...
                        <div class="entry-header">
                            ...
                            <?php if( $based_on == 'views' && $view_count ){ ?>
                                <span class="view-count"><?php echo esc_html( $this->edward_blog_get_views( get_the_ID() ) );?></span>
                            <?php }
            ...
            <?php
            $html = ob_get_clean();
            echo apply_filters( 'edward_blog_widget_popular_post_widget_filter', $html, $args, $instance );
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'num_post' ) ); ?>"><?php esc_html_e( 'Number of Posts', 'edward-blog' ); ?></label> 
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'based_on' ) ); ?>"><?php esc_html_e( 'Popular based on:', 'edward-blog' ); ?></label>
            <select id="<?php echo esc_attr( $this->get_field_id( 'based_on' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'based_on' ) ); ?>" class="based-on">
                <option value="views" <?php selected( $based_on, 'views' ); ?>><?php esc_html_e( 'Post Views', 'edward-blog' ); ?></option>
                <option value="comments" <?php selected( $based_on, 'comments' ); ?>><?php esc_html_e( 'Comment Count', 'edward-blog' ); ?></option>
            </select>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_thumbnail' ) ); ?>"><?php esc_html_e( 'Show Post Thumbnail', 'edward-blog' ); ?></label>
        </p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_postdate' ) ); ?>"><?php esc_html_e( 'Show Post Date', 'edward-blog' ); ?></label>
        </p>

        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>">
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'target' ) ); ?>" type="checkbox" value="1" <?php echo checked($target,1);?> /><?php esc_html_e( 'Open in New Tab', 'edward-blog' ); ?> </label>
        </p>
    ...
    public function update( $new_instance, $old_instance ) {
        ...
        $instance = array();
        $instance['title']          = ! empty( $new_instance['title'] ) ? sanitize_text_field( $new_instance['title'] ) : __( 'Recent Posts', 'edward-blog' );
